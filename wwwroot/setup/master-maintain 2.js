...
$(document).ready(function () {
    ...
    $("#EditData").click(function () {//編輯資料
        ...
        if (SID === "367252115657308") {
            window.location.href = `/${PROJECT_NAME}/${kanbanRoute}/setup/master-maintain-setting.html?ACTION=EDIT&SID=${EditPkey}&MODULE_TYPE=ADM&MODULE_NAME=SMART_MASTER&LEVEL=L3&BUTTON=A`
        }
        // Layout設定
        if (SID === "369761779674442") {
            window.location.href = `/${PROJECT_NAME}/${kanbanRoute}/setup/layout-setting/layoutSetting.html?action=edit&SID=${EditPkey}&MODULE_TYPE=ADM&MODULE_NAME=LAYOUT&LEVEL=L2&BUTTON=A`
        }
    ...
    $("#AddData").click(function () {//新增資料
        // 主檔維護設定
        if (SID === "367252115657308") {
            window.location.href = `/${PROJECT_NAME}/${kanbanRoute}/setup/master-maintain-setting.html?ACTION=ADD&MODULE_TYPE=ADM&MODULE_NAME=SMART_MASTER&LEVEL=L3&BUTTON=A`
        }
        // Layout設定
        if (SID === "369761779674442") {
            window.location.href = `/${PROJECT_NAME}/${kanbanRoute}/setup/layout-setting/layoutSetting.html?action=add&MODULE_TYPE=ADM&MODULE_NAME=LAYOUT&LEVEL=L2&BUTTON=A`
        }
        var AddRequiredC = [];
        $("#ADDContent").empty();
        for (var i = 0; i < AllEditTitle.split(",").length; i++) {
            ...
            if ($.inArray(AllEditTitle.split(",")[i], EditTitle) >= 0) {
                ...
                switch (EditType[a].toUpperCase()) {
                    case 'TEXT':
                        ...
                        else if (EditFieldType[a] === 'int') {
                            //6/11 int ui友善
                            $("#ADDContent").append('<div class="d-flex justify-content-between mb-1">' + '<div class="Lang col-4 text-end">' + AllEditTitle.split(",")[i] + '</div>' + '<input placeholder="Please enter a number." class="col-7" id="' + AllEditTitle.split(",")[i] + '" name="' + editFieldType(EditFieldType[a]) + '"  autocomplete="off"  />' + '</div>');
                            $('#' + AllEditTitle.split(",")[i]).on('focusout', function () {
                                if (isNaN($(this).val())) {
                                    customAlertError('Please enter a number.')
                                    $(this).val('')
                                }
                            })
                        }
