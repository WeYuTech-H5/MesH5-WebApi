...
$(document).ready(function () {
    ...
    $("#AddData").click(async function () {
        ...
        // 🔽 下面這些邏輯，都放進這裡來，保證資料準備好才做

        // 綁定 TOL_SID 變更事件（只綁一次）
        $(document).off('change', '#TOL_SID').on('change', '#TOL_SID', function () {
            const val = $(this).val();
            const item = CURDATA.Grid_Data.find(obj => obj.TOL_SID == val);
            $("#CUR_CAV").val(item ? item.CUR_CAV : '');
        });

        // 初始化 CUR_CAV
        const initVal = $("#TOL_SID").val();
        const initItem = CURDATA.Grid_Data.find(obj => obj.TOL_SID == initVal);
        $("#CUR_CAV").val(initItem ? initItem.CUR_CAV : '');
                    ...
                    for (var i = 0; i <= result.length; i++) {
                        for (var a in result[0]) {
                            ...
                            if (x == 0) {
                                ...
                                SSOhtml += result[i][a] + ',  ';//select 的第二個
                                x = 2;
                            } else {
                                SSOhtml += result[i][a] + '</option>'
                                x = 0
