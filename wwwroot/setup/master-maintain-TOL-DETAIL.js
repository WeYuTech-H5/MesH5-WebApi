            ...
            if ($.inArray(AllEditTitle.split(",")[i], EditTitle) >= 0) {//如果有屬於要編輯的title的話
                ...
                switch (EditType[a].toUpperCase()) {
                    ...
                    case 'SELECT':
                        ...
                        //進階搜尋下拉式 2023-05-03
                        // 建立 select 欄位
                         const sortItems = ['TOL_NO', 'PART_NO']

                        if (sortItems.includes(AllEditTitle.split(",")[i])) {
                ...
                switch (EditType[a].toUpperCase()) {
                    case 'TEXT':
                        if (['MSID', 'MASTER_SID'].includes(AllEditTitle.split(",")[i])) {
                            ...
                            if(ALLTitle.split(",")[i] ==='UNIT'){
                                $("#ADDContent").append('<div class="d-flex justify-content-between mb-1">' + '<div class="Lang col-4 text-end">' + AllEditTitle.split(",")[i] + '</div>' + '<input value="PCS" class="col-7" id="' + AllEditTitle.split(",")[i] + '" name="' + editFieldType(EditFieldType[a]) + '"  autocomplete="off"  />' + '</div>');
                                console.log('unit')
                            }else{
...
//Br客製
function SelectOption_sort(SSO) { //選項賦值
    var SSOhtml = "";
    SSOhtml = '<option value="">-</option>';
    ...
    if (SSO.indexOf('SQL') >= 0) {
        $.ajax({
            ...
            success: function (result) {
                ...
                result = result.data;
                result.sort((a, b) => {
                    const key = Object.keys(a)[1]; // 第 2 個屬性名
