...
$(document).ready(function () {
    ...
    $("#EditData").click(async function () {//編輯資料
        CURDATA = await getGridDataV2('385207208233661');
        MODATA = await getGridDataV2('385296863903798');
        SHIFTDATA = await getGridDataV2('385376946253405');

        $(document).ready(function () {
            $('#PART_NO,#PART_SPEC,#WO_QUANTITY')
                .prop('readonly', true)
        ...
        for (var i = 0; i < AllEditTitle.split(",").length; i++) {
            ...
            if ($.inArray(AllEditTitle.split(",")[i], EditTitle) >= 0) {//如果有屬於要編輯的title的話
                ...
                switch (EditType[a].toUpperCase()) {
                    case 'TEXT':
                        let fieldId = AllEditTitle.split(",")[i];
                        let fieldValue = EditFiledData[i]; // 要帶入的編輯值

                        if (['SCHEDULE_START_TIME', 'REPORT_END_TIME'].includes(fieldId)) {
                            // 使用 flatpickr 日期時間選擇器
                            $("#EDDContent").append(
                                '<div class="d-flex justify-content-between mb-1">' +
                                '<div class="Lang col-4 text-end">' + GetLangDataV2(fieldId) + '</div>' +
                                '<input class="flatpickr-input col-7" type="text" id="' + fieldId + '" name="' + editFieldType(EditFieldType[a]) + '" autocomplete="off" />' +
                                '</div>'
                            );
                            ...
                            flatpickr('#' + fieldId, {
                                ...
                                dateFormat: "H:i",    // 時:分格式 (24小時)
                                time_24hr: true ,      // 24小時制
                                defaultDate: fieldValue || null // 編輯時預設值
                            });
                            ...
                            $("#EDDContent").append(
                                '<div class="d-flex justify-content-between mb-1">' +
                                '<div class="Lang col-4 text-end">' + GetLangDataV2(fieldId) + '</div>' +
                                '<input class="col-7" id="' + fieldId + '" name="' + editFieldType(EditFieldType[a]) + '" autocomplete="off" />' +
                                '</div>'
                            );

                            $("#" + fieldId).val(fieldValue); // 帶入資料
                        }
                        break;
                    ...
                    case 'DATE':
                        ...
                        $("#" + AllEditTitle.split(",")[i] + "").val(EditFiledData[i]?.split("T")[0]);
                        break;
                    case 'SELECT':
                        ...
                        // 監聽 TOL_SID 改變事件
                        $(document).off('change', '#TOL_SID').on('change', '#TOL_SID', function () {
                            const tolSid = $(this).val();

                            // 找到對應的 PART_NO 和 CUR_CAV
                            const item = CURDATA.Grid_Data.find(obj => obj.TOL_SID == tolSid);
                            $("#SCHEDULE_CUR_CAV").val(item?.CUR_CAV || '');
                            $("#SCHEDULE_CYCLE_TIME").val(item?.DEFAULT_CYCLE_TIME || '');
                            $("#PART_NO").val(item?.PART_NO || '').prop("readonly", true);
                            $("#PART_SPEC").val(item?.PART_SPEC || '').prop("readonly", true);
                            
                            // 過濾 MODATA 中對應的 MO 工單
                            const moSelect = $('#WO_SID');
                            moSelect.empty(); // 清空現有選項

                            const filteredMOs = MODATA.Grid_Data.filter(obj => obj.TOL_SID == tolSid);
                            filteredMOs.forEach(obj => {
                                moSelect.append(`<option value="${obj.WO_SID}">${obj.WO}</option>`);
                            });

                            // 預設選第一個，並觸發 change 帶出數量
                            if (filteredMOs.length > 0) {
                                moSelect.val(filteredMOs[0].WO_SID).trigger('change');
                            } else {
                                $('#WO_QUANTITY').val('');
                            }
                        });
                        ...
                        // 監聽 MO 工單選擇變動，自動帶出數量
                        $(document).off('change', '#WO_SID').on('change', '#WO_SID', function () {
                            const woSid = $(this).val();
                            const moItem = MODATA.Grid_Data.find(obj => obj.WO_SID == woSid);
                            $('#WO_QUANTITY').val(moItem?.QUANTITY || '');
                        });
                        ...
                        function updateShiftTimes() {
                            ...
                            // 找出對應的 shift 物件
                            const shift = SHIFTDATA.Grid_Data.find(obj => obj.SHIFT_TYPE.trim() === shiftType.trim());
                            if (!shift) return;

                            const reportDateStr = $('#SCHEDULE_DATE').val();
                            if (!reportDateStr) return;

                            // 將 REPORT_DATE 字串轉成 Date 物件（不含時間）
                            const baseDate = new Date(reportDateStr.replace(/-/g, '/'));
                            ...
                            // 根據 baseDate 和時間字串(如 "08:00") 組成完整 datetime 字串，並且可跨日 (addDay = 0或1)
                            function combineDateTime(baseDate, timeStr, addDay = 0) {
                                const [hh, mm] = timeStr.split(':').map(Number);
                                const d = new Date(baseDate);
                                if (addDay === 1) d.setDate(d.getDate() + 1);
                                d.setHours(hh, mm, 0, 0);
                                return formatDateTime(d);
                            }

                            // 填入對應欄位
                            $('#SHIFT_CLASS_1').val(shift.SHIFT_CLASS_1);
                            $('#SHIFT_CLASS_2').val(shift.SHIFT_CLASS_2);

                            $('#SHIFT_START_TIME_1').val(combineDateTime(baseDate, shift.START_TIME_1, shift.START_DATE_1));
                            $('#SHIFT_END_TIME_1').val(combineDateTime(baseDate, shift.END_TIME_1, shift.END_DATE_1));

                            $('#SHIFT_START_TIME_2').val(combineDateTime(baseDate, shift.START_TIME_2, shift.START_DATE_2));
                            $('#SHIFT_END_TIME_2').val(combineDateTime(baseDate, shift.END_TIME_2, shift.END_DATE_2));
                        }

                        // 綁定事件
                        $(document).off('change', '#SHIFT_TYPE').on('change', '#SHIFT_TYPE', updateShiftTimes);
                        $(document).off('change', '#SCHEDULE_DATE').on('change', '#SCHEDULE_DATE', updateShiftTimes);




                        // 初始觸發一次，帶入預設值
                        $("#TOL_SID").trigger('change');
        ...
        //     }
        // })
        $(document).on('input', '#SCHEDULE_CUR_CAV, #TARGET_SHOT', function () {
            var curCav = Number($("#SCHEDULE_CUR_CAV").val()) || 0;
            var targetShot = Number($("#TARGET_SHOT").val()) || 0;
            $("#TARGET_PCS")
                .val(curCav * targetShot)
        });
        $("#progress,#loading").fadeOut(1000);
    })//編輯資料
    var CBAValue = [];
        ...
        for (var i = 0; i < AllEditTitle.split(",").length; i++) {
            ...
            if ($.inArray(AllEditTitle.split(",")[i], EditTitle) >= 0) {
                ...
                switch (EditType[a].toUpperCase()) {
                    case 'TEXT':
                        let fieldId = AllEditTitle.split(",")[i];
                        if (['SCHEDULE_START_TIME', 'SCHEDULE_END_TIME'].includes(fieldId)) {
                            // 改成 flatpickr 日期時間選擇器 (type=text)
                            $("#ADDContent").append(
                                '<div class="d-flex justify-content-between mb-1">' +
                                '<div class="Lang col-4 text-end">' + GetLangDataV2(fieldId) + '</div>' +
                                '<input class="flatpickr-input col-7" type="text" id="' + fieldId + '" name="' + editFieldType(EditFieldType[a]) + '" autocomplete="off" />' +
                        ...
                        } else if (['MSID', 'MASTER_SID'].includes(fieldId)) {
                            $("#ADDContent").append(
                                '<div class="d-flex justify-content-between mb-1">' +
                                '<div class="Lang col-4 text-end">' + fieldId + '</div>' +
                                '<input class="col-7" id="' + fieldId + '" name="' + editFieldType(EditFieldType[a]) + '" autocomplete="off" disabled="disabled" />' +
                                '</div>'
                            );
                        } else {
                            $("#ADDContent").append(
...
async function getGridDataV2(SID) {
