...
// 用到的SID
const sidMapping = {
  EQPListSid: '383582888640260', //EQP下拉清單選項 O
  OEEStandardSid: '350057276290452', //OEE標準&對應顏色
  statusLegendColorSid: '238864537250988', //機況甘特圖例顏色 
  // IOTCounterSid: '358706730326253', //IOT開合次數 O
  outputSid: '357998862096973', //柱狀圖
  EQPGanttSid: '305991215183875', //機況甘特圖
  CICOGanttSid: '358003490890214', //進出站甘特圖
  CICOTableSid: '386602456343323', //進出站table
  EQPInfoSid: '380633150300975',
  // EQP_IOT_DAY: '374930113120279'
  EQP_IOT_DAY: '375123226523945'
}

let dashboardTitle = `${EQP_TYPE}_Dashboard`;
...
async function GetEQPInfonew() {
  //替換SQL
  let condition = {
    "[EQP_NO]": EQP_NO,
    "[SHIFT_DAY]": SHIFT_DAY
  }
  let EQPInfoData = await getData(sidMapping.EQP_IOT_DAY, condition);

  if (EQPInfoData.rows.length > 0) {
    // 塞資料
    let DAY_OUTPUT = EQPInfoData.rows[0].DAY_IOT_COUNT == '' ? 'N/A' : EQPInfoData.rows[0].DAY_IOT_COUNT.split('.')[0]
    let DAY_UPTIME = EQPInfoData.rows[0].DAY_RUN_TIME == '' ? 'N/A' : EQPInfoData.rows[0].DAY_RUN_TIME.split('.')[0]
    let DAY_AVG = EQPInfoData.rows[0].DAY_IDLE_TIME == '' ? 'N/A' : EQPInfoData.rows[0].DAY_IDLE_TIME.split('.')[0]

    let NIGHT_OUTPUT = EQPInfoData.rows[0].NIGHT_IOT_COUNT == '' ? 'N/A' : EQPInfoData.rows[0].NIGHT_IOT_COUNT.split('.')[0]
    let NIGHT_UPTIME = EQPInfoData.rows[0].NIGHT_RUN_TIME == '' ? 'N/A' : EQPInfoData.rows[0].NIGHT_RUN_TIME.split('.')[0]
    let NIGHT_AVG = EQPInfoData.rows[0].NIGHT_IDLE_TIME == '' ? 'N/A' : EQPInfoData.rows[0].NIGHT_IDLE_TIME.split('.')[0]

    let TOTAL_OUTPUT = EQPInfoData.rows[0].TOTAL_IOT_COUNT == '' ? 'N/A' : EQPInfoData.rows[0].TOTAL_IOT_COUNT.split('.')[0]
    let TOTAL_UPTIME = EQPInfoData.rows[0].TOTAL_RUN_TIME == '' ? 'N/A' : EQPInfoData.rows[0].TOTAL_RUN_TIME.split('.')[0]
    let TOTALT_AVG = EQPInfoData.rows[0].TOTAL_IDLE_TIME == '' ? 'N/A' : EQPInfoData.rows[0].TOTAL_IDLE_TIME.split('.')[0]
    $('#COUNTER_DAY').html(`${DAY_OUTPUT}`);
    $('#OUTPUT_DAY').html(`${DAY_UPTIME}`);
    $('#LOSS_DAY').html(`${DAY_AVG}`);

    $('#COUNTER_NIGHT').html(`${NIGHT_OUTPUT}`);
    $('#OUTPUT_NIGHT').html(`${NIGHT_UPTIME}`);
    $('#LOSS_NIGHT').html(`${NIGHT_AVG}`);

    $('#COUNTER_TOTAL').html(`${TOTAL_OUTPUT}`);
    $('#OUTPUT_TOTAL').html(`${TOTAL_UPTIME}`);
    $('#LOSS_TOTAL').html(`${TOTALT_AVG}`);
...
//   // 初始化 total_count 和班次數據
//   let total_count = 0;
//   let dayShiftCount = 0;
//   let nightShiftCount = 0;

//   // 確保在沒有 DAY_SHIFT 或 NIGHT_SHIFT 時，值為 0
//   EQPInfoData3.rows.forEach(function (e) {
//     let count = parseInt(e.IOT_COUNT) || 0; // 如果 IOT_COUNT 為 NaN, 設為 0
//     switch (e.SHIFT_NO.toUpperCase()) {
//       case "DAY_SHIFT":
//         dayShiftCount = count;
//         break;
//       case "NIGHT_SHIFT":
//         nightShiftCount = count;
//         break;
//     }
//   });

//   // 設定顯示的值
//   $('#COUNTER_DAY').html(dayShiftCount);
//   $('#COUNTER_NIGHT').html(nightShiftCount);

//   // 更新總計數
//   total_count = dayShiftCount + nightShiftCount;
...
async function GetEQPOutPut() {
  ...
  $.ajax({
    ...
    data: {
      Charts: JSON.stringify(outputStructer.Charts), SQL: outputStructer.MasterSql, AddedSQL: outputStructer.AddedSql, Conds: JSON.stringify(outputStructer.Conditions), GridFieldType: JSON.stringify(outputStructer.GridFieldType),
    ...
    "columnDefs": [
      ...
      },
      {
        ...
        render: function (data, type, row, meta) {
          const sid = row[1]   // SID 在第幾欄就要改多少
          return '<a href="../../../MES-WIP/OPI/schedule/wo-schedule.html?WIP_OPI_WDOEACICO_HIST_SID=' + sid + '&DEPT_NO=' + EQP_TYPE + '&SHIFT_DAY=' + SHIFT_DAY + '">' + data + '</a>';
        }
      }
    ],
    "dom": 'frt',
    ...
    $.ajax({
      ...
      success: function (msg) {
        ...
        $.ajax({
          type: 'POST',
          url: window.location.protocol + '//' + default_ip + '/' + default_WebSiteName + '/QueryAndReport/QueryReportDataHandler4FullScreen.ASHX?DataType=Grid&DBLink=SELF',
          data: { Charts: JSON.stringify(jsonObj.Charts), SQL: jsonObj.MasterSql, AddedSQL: jsonObj.AddedSql, Conds: JSON.stringify(jsonObj.Conditions), GridFieldType: JSON.stringify(jsonObj.GridFieldType), SID: SID, rows: 9999 },
          success: function (msg) {
...
function goBack() {
  window.location.href = "../L1/fab.html?EQP_TYPE=" + `${EQP_TYPE}` + "&SHIFT_DAY=" + `${SHIFT_DAY}`; // 跳轉到指定 URL
}