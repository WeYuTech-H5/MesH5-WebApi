...
let AirconditionButtonHTML = `<button id="AirconditionButton" class="btn btn-outline-primary"><img src="../img/weyu/air-conditioner.svg" alt="Query"><span class="Lang">${GetLangDataV2('AirCondition mgt.')}</span></button>`
...
//機台拖拉查詢按鈕
let draggingHTML = `<button id="draggingButton" class="btn btn-outline-primary"><img src="../img/weyu/complain-handling.svg" alt="Query"><span class="Lang">${GetLangDataV2('View')}</span></button>`
//機台拖拉 編輯/上傳 圖片
let draggingUploadIMGHTML = `<button id="draggingUploadIMG" class="btn btn-outline-primary"><img src="../img/weyu/complain-handling.svg" alt="Query"><span class="Lang">${GetLangDataV2('View')}</span></button>`
//機台拖拉 刪除 配置圖
let draggingDelHTML = `<button id="draggingDel" class="btn btn-outline-primary"><img src="../img/weyu/complain-handling.svg" alt="Query"><span class="Lang">${GetLangDataV2('View')}</span></button>`


// 每個查詢的客製選項
let SID_BUTTON_REALATION = [
    {SID:'331903774523639', MAINTAIN_ON:true, QUERY_ON:true },  //L1 設備維護
    {SID:'334339875776290', STATUS_ON:true, MOUDLES_ON:true, MAINTAIN_ON:true, QUERY_ON:true },  //L1 模具管理
    {SID:'335037058616164', COMPLAINT_ON:true },  //L1 客訴 //加按鈕2.
    ...
    {SID:'334579037386718', MAINTAIN_ON:true, QUERY_ON:true, REFRIGERANT_ON:true, AIRCONDITION_ON:true},  //L2 ESG 冷卻設備
    ...
    {SID:'343128535020582', dragging_ON:true },  //L1 機台拖拉查詢
...
function CreateCustomButton(SID) {
    SID_BUTTON_REALATION.forEach(buttonRelation => {
        if (buttonRelation.SID === SID) {
            ...
            if (buttonRelation.AIRCONDITION_ON) {$('.divider').after(AirconditionButtonHTML)}
            ...
            if (buttonRelation.dragging_ON) {$('.divider').after(draggingHTML)}
...
$('#QueryButton').click(function(){
    switch(currentSID){
        ...
        case '336407516436566': // 部門報工
            url = window.location.protocol+"//"+default_ip+"/"+PROJECT_NAME+"/"+kanbanRoute+'/'+'query/check-in-query-all.html?SID=337022255306774&MODULE_NAME=PRODUCTION&LEVEL=L4&BUTTON=D';
            break;
        case '337439812260107': // 人員報工
            url = window.location.protocol+"//"+default_ip+"/"+PROJECT_NAME+"/"+kanbanRoute+'/'+'query/check-in-query-all.html?SID=337972354570235&MODULE_NAME=PRODUCTION&LEVEL=L4&BUTTON=E';
            break;
    }
    window.location.href = url;
})
...
//ESG
$('#AirconditionButton').click(function(){
...
$('#ComplaintHandling').click(function(){
    ...
    url = window.location.protocol+"//"+default_ip+"/"+PROJECT_NAME+"/"+kanbanRoute+'/'+"setup/master-maintain.html?CUSTOMER_COMPLAINTS_SID="+thisSId+"&SID=335036498080839&LEVEL=L3&MODULE_NAME=CUSTOMER_COMPLAINTS&BUTTON=A",
...
$('#Report').click(function(){
    switch(currentSID){
        case '336407516436566':
            url = window.location.protocol+"//"+default_ip+"/"+PROJECT_NAME+"/"+kanbanRoute+'/'+'setup/check-in-query.html?SID=338297847263983&MODULE_NAME=PRODUCTION&LEVEL=L4&BUTTON=D&DEPT_NO='+findNO('DEPT_NO')
            break;
        case '337439812260107':
            url = window.location.protocol+"//"+default_ip+"/"+PROJECT_NAME+"/"+kanbanRoute+'/'+'setup/check-in-query.html?SID=338297914450237&MODULE_NAME=PRODUCTION&LEVEL=L4&BUTTON=E&DEPT_NO='+findNO('DEPT_NO')
            break;
    }
    window.location.href = url;
})
...
url = window.location.protocol+"//"+default_ip+"/"+PROJECT_NAME+"/"+kanbanRoute+'/'+ 'query/map.html?EQPLAYOUT_SID='+thisSID_CODE;
    window.location.href = url;
