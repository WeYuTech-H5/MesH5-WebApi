...
$("#OPI_CAV").on("input", function() {
  ...
  // ^\d*\.?\d*$ 代表任意數字，最多一個小數點
  if (!/^\d*\.?\d*$/.test(val)) {
    alert("Please enter a non-negative number (digits and at most one decimal point)!");
    $(this).val(lastVal);
  } else {
    lastVal = val;
  }
