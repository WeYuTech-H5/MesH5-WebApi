    ...
    try{
        ...
        for(var i=0;i<carddata.Grid_Data.length;i++){ //carddata.Grid_Data.length
            ...
            tmpcardhtml += '<div class="wrapperUp text-white">';
                ...
                tmpcardhtml += `<div style="height:50px"><canvas id="${chartId}"></canvas></div>`;
                tmpcardhtml += `<div style="height:50px"><canvas id="${chartId}_test"></canvas></div>`
...
function slideCarousel() {
    ...
    // 2023-11-16新增 定期輪播功能
    const intervalTime = 15000; // 每5秒輪播一次（根據需要調整）
