...
$(document).ready(function () {
    ...
    var SID = Request["SID"];
    var DEPT_NO = Request["DEPT_NO"] || null;
    //backUrl參數
    ...
    for (var i = 0; i < ConditionIndexValue; i++) {
        switch (QueryInputType[i]) {
            case "KeyIn":
                ...
                if(["MASTER_SID"].includes(QueryCaption[i])){
    ...
    $("#AddData").click(function () {
        ...
        //部門報工
        if(SID=="336410202180277"){
            url = window.location.protocol+"//"+default_ip+"/"+PROJECT_NAME+"/"+kanbanRoute+'/'+"wip/department-check-in.html?DEPT_NO="+DEPT_NO
            window.open(url, '_blank');
        }
        //人員報工
        else if(SID=="337440897016170"){
            url = window.location.protocol+"//"+default_ip+"/"+PROJECT_NAME+"/"+kanbanRoute+'/'+"wip/user-check-in.html?DEPT_NO="+DEPT_NO
            window.open(url, '_blank');
        }
    ...
    $("#EditData").click(function () {//編輯資料
        if(SID=="336410202180277"){
            url = window.location.protocol+"//"+default_ip+"/"+PROJECT_NAME+"/"+kanbanRoute+'/'+"wip/department-check-in.html?DEPARTMENT_WO_HIST_SID="+rowDataTest.DEPARTMENT_WO_HIST_SID
            window.open(url, '_blank');
        }
    })//編輯資料
    ...
    // 2023-12-28 新增 刪除按鈕
    var Delval = "";
    var SidField = "DEPARTMENT_WO_HIST_SID";
    $('#DeleteData').click(function () {
        $("#DELETEContent").empty();
        $("#DELETEContent").append('<div style="font-size:30px">是否要刪除?</div>')
        Delval = rowData.DEPARTMENT_WO_HIST_SID
    });
    ...
    function TableSet() {//DataTable管理
        table = $('#example').DataTable({
            // 語言選項
            language: {
                emptyTable: 'Table is empty now, click [Add] to create data'
            },
            data: DataSet,
            dom: 'frt',
            "lengthMenu": [perPData],
    ...
    function getRowData(rowHtml) {
        ...
        let columnValue = TableData.row('.selected').data()

        let columnNames = TableData.columns().header().to$().map(function() {
            return $(this).text();
        }).get();

        rowDataTest=[];
        for(let i=0;i<columnNames.length;i++){
            rowDataTest[columnNames[i]] = columnValue[i]
        }
    ...
    function SelectRowOnOff(OPEN) {
        ...
        $("#InvalidData").prop("disabled", OPEN);
    }
    ...
    function SetMSquery(SID){
        ...
        if(IsMsqueryChange){
            switch(SID){
                case "336410202180277":
                    let DEPT_NO = Request["DEPT_NO"];
                    DEPT_NO = DEPT_NO.replace(/'/g, "''")
                    MSQuery = MSQuery.replace('WHERE',"WHERE DEPT_NO='"+DEPT_NO+"' AND");
                    break;
                case "337440897016170":
                    let DEPT_NO2 = Request["DEPT_NO"];
                    DEPT_NO2 = DEPT_NO2.replace(/'/g, "''")
                    MSQuery = MSQuery.replace('WHERE',"WHERE DEPT_NO='"+DEPT_NO2+"' AND");
                    break;
