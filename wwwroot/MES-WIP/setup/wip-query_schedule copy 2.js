...
$(document).ready(function () {
    ...
    var SHIFT_DAY = Request["SHIFT_DAY"];
    ...
    for (var i = 0; i < Dpacker.length; i++) {
        ...
        if (elId === 'QDSSHIFT_DAY' || elId === 'QDESHIFT_DAY') {
            defaultDate = SHIFT_DAY; // 預設帶入 SHIFT_DAY 的日期
        }
        flatpickr('#' + Dpacker[i], {
            dateFormat: "Y-m-d",
            allowInput: true,
    ...
    $("#EditData").click(function () {//編輯資料
        url = window.location.protocol + "//" + default_ip + "/" + PROJECT_NAME + "/" + kanbanRoute + "/MES-WIP/OPI/schedule/wo-schedule.html?WIP_OPI_WDOEACICO_HIST_SID=" + rowArray[1] + "&DEPT_NO=" + OPERATION + "&SHIFT_DAY=" + SHIFT_DAY
        window.location.href = url
    })//編輯資料
    var CBAValue = [];
    $("#AddData").click(function () {
        url = window.location.protocol + "//" + default_ip + "/" + PROJECT_NAME + "/" + kanbanRoute + "/MES-WIP/OPI/checkin/wo-check-in.html?DEPT_NO=" + DEPT_NO
        window.location.href = url
    })
    ...
    function SelectRowOnOff(OPEN) {
        ...
        if (OPEN) {
            $("#EditData").hide();
            $("#AddData").show();
            $("#AddData").prop("disabled", true);  // 禁用，無法按
        }
        else {
            $("#EditData").show();
            $("#AddData").hide();
