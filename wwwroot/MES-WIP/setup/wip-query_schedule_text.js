...
$(document).ready(async function () {
    ...
    let tabledata = await fetchData(SID, OPERATION);
...
async function fetchData(SID, OPERATION) {
    // 查詢條件
    let conditions = {
        TABLE_NAME: "GET_V_WIP_OPI_WDOEACICO_HIST_SCHEDULE",
        VALUE: [OPERATION],
        CON: {
            Field: [],
            Oper: [],
    ...
    const response = await getFunctionGridData(SID, conditions)
