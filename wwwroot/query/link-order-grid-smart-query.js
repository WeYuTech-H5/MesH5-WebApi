...
$(document).ready(function () {
    ...
    var DEL_FLAG = Request["DEL_FLAG"] || 'N';
    ...
    $.each(QueryStructer.sColModels4EasyUI, function (k, v) {
        $.each(v, function (ka, va) {
            switch (ka) {
                case "title":
                    $('#addtitle').append('<th id="Title' + va + '" style="width:' + v['width'] + 'px">' + va + '</th>');
                    TitleArray.push(va);
                    break;
                case "field":
            ...
            case "Select":
                switch (QueryDataType[i]) {
                    case "DateTime":
                        ...
                        $("#QueryContent").append('<div class="Lang col-md-12">' + GetLangDataV2(QueryCaption[i]) + ' ' + GetLangDataV2('start') + '</div>' + '<input class="col-md-12" type="button" id="QS' + QueryField[i] + '"  autocomplete="off"/><p>');
                        ...
                        if (QueryOper[i] == "between") {
                            $("#QueryContent").append(GetLangDataV2(QueryCaption[i]) + ' ' + GetLangDataV2('end') + '<input class="col-md-12" type="button" id="QE' + QueryField[i] + '"  autocomplete="off"/><p>')
    ...
    function GetPageInfo() {
        $.ajax({
            ...
            success: function (result) {
                ...
                $("#totalnum").text("共 " + SqlTimes + " 頁(" + DataSummary + "筆資料)");
                if (SqlTimes == 1) {
    ...
    $('#example tbody').on('click', 'tr', function () { //選擇事件
        if($(this).hasClass('selected')){
            ...
            SelectRowOnOff(true) // 取消選取

        }else{
            $('#example tbody tr').removeClass('selected');
            $(this).toggleClass('selected');
            SelectRowOnOff(false) // 選取
    ...
    $("#ExportExcel").click(function () {
        ...
        $.ajax({
            ...
            data: { funcName: "Exportxlsx", FileName: $("#midtitle").text(), SQL: MSQuery, ColName: c, CheckCol: b, mode: "GridDataSmartQuery",SID:SID },//ExportExcel
            //dataType: 'json',
            async: false,
            success: function (result) {
    ...
    function replaceTD() {
        ...
        $.each(HyperLinkContent, function (k, v) {
            ...
            for (var i = 0; i < table.column(0).data().length; i++) {
                ...
                if(DEL_FLAG=='Y'){
                    c = c.replace('file-upload.html','file-upload-delete.html');
                }
                ...
                // if(SID=="326050155480875"){
                // //如果有加排序 2 這裡要改 
                // key_index = DataIndexSet.indexOf(b[2].substring(1, b[2].length));

                // c = c.replace(b[2], DataHideSet[i][DataIndexSet2[key_index]]);

                // key_index = DataIndexSet.indexOf(b[1].substring(1, b[1].length));

                // c = c.replace(b[1], DataHideSet[i][DataIndexSet2[key_index]]);
                // }
